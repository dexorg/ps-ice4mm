 ...
 */
/*global dexit, tinycolor, ko */

/**
 *
 * @param {object} params - parameters
 * @param {string} params.epId - engagement pattern identifier
 * @param {object} params.tp - BCC touchpoint
 * @param {string} [params.name] - Name to display
 * @param {string} [params.icon] - icon to us
 * @param {string} [params.colour] - background colour to show
 *
 */
dexit.app.ice.edu.components.MerchChannelWidgetVM = function (params) {

    this.epId = params.epId;
    this.tp = params.tp;

    var enable = (this.epId && this.tp ? true : false);
    ...
    if (params.name) {
        ...
        this.name = ko.observable('Product');
    }

    if (params.image) {
        this.image('images/'+ params.image);
    } else {
        if (params.icon) {
    ...
    if (params.colour) {
        this.colour = ko.observable('#' + params.colour);
    } else {
        this.colour = ko.observable('#85bb65');
    }

    //30% darker than background colour
    var borderColour = tinycolor(this.colour()).darken(30).toString();

    this.border = ko.observable('2px solid '+borderColour);
 ...
 *
 * @param {object} params
 * @param {ko.observable} params.bccActive - passed in from parent VM as reference
 * @param {boolean} [params.showControls=false] - if true, show controls
 * @param {boolean} [params.previewMode] - reference to parent VM
 * @param {ko.observable<boolean>} [params.showPlayer] - reference to observable of when to show/hide player
 * @param {function} params.backFn - reference to parent VM
 */
dexit.app.ice.edu.components.BCCPlayerVM = function (params) {

    //make sure a value is set to true or false
    var pm = ( (params.previewMode && params.previewMode === true) ? params.previewMode : false);

    this.showPlayer = params.showPlayer;

    this.showControls = (params.showControls && params.showControls === true ? params.showControls : false);
    /**
     * If set may show different controls or playback may look different
     */
    this.previewMode = ko.observable(pm);
    this.bccActive = params.bccActive;
 ...
 *
 * @param {object} params
 * @param {string} params.prefix - prefix to append
 * @param {string[]} [params.iconList] - List of icons (default list of icons for FA 4.7.0)
 * @param {ko.observableArray} availableWidgetIntelligence
 * @constructor
 */
dexit.app.ice.edu.components.CSSIconPickerVM = function (params) {
    ...
    var prefix = (params && params.prefix ? params.prefix : 'fa ' );
    var iconList = (params && params.iconList ? params.iconList : defaultList);
    //transform to have label

    var iconFields = iconList.map(function (val) {
        return {
            label: '<i class="'+prefix+val+'"></i>',
            value:prefix+val
        };
    });

    self.iconList = ko.observableArray(iconFields);

    self.chosen1Icon = ko.observable(iconFields[0].value);
    self.chosen2Icon = ko.observable(iconFields[0].value);
    self.chosen3Icon = ko.observable(iconFields[0].value);
...
/**
 * Registers knockout components using the default knockout component loader
 * Note: will always check that component name is not already registered
 * @param {object} [koLib=window.ko] - pass in reference to knockout library (useful for testing)
 */
dexit.app.ice.edu.components.register = function(koLib) {

    var ko = koLib || window.ko;

    if (ko.components.isRegistered('merch-channel-widget') !== true) {

        //register ko.widget for merchandising channel
        ko.components.register('merch-channel-widget', {
            viewModel: dexit.app.ice.edu.components.MerchChannelWidgetVM,
            template: {element: 'merch-channel-widget-template'}
        });
    }


    if (!ko.components.isRegistered('bcc-player')) {

        //register player
        ko.components.register('bcc-player', {

            viewModel: dexit.app.ice.edu.components.BCCPlayerVM,
            template: {element:'bcc-player-template'}
        });

    }

    if (!ko.components.isRegistered('color-picker')) {
        ko.components.register("color-picker", {
            template: { element:"tmpl-colorPicker"},
            viewModel: function(params) {
                var self = this;
                self.color = params.color;
                self.contrastColor = ko.pureComputed(function() {
                    return contrastColor(self.color());
                })
                self.tempColor = ko.observable(self.color());
                self.contrastTempColor = ko.pureComputed(function() {
                    return contrastColor(self.tempColor());
                })
                self.colorPalette = params.colorPalette || ["#FF0000", "#00FF00", "#0000FF", "#FFFF00", "#00FFFF", "#FF00FF"];
                self.extraColorPalette = params.extraColorPalette || ["#FF0000", "#00FF00", "#0000FF", "#FFFF00", "#00FFFF", "#FF00FF",
                ...
                self.isExtraPaletteVisible = ko.observable(false);
                self.colors = ko.observableArray(self.colorPalette);
                ...
                self.setTempColor = function() {
                    self.tempColor(this);
                }
                self.confirmTempColor = function () {
                    self.color(self.tempColor());
                    self.isPickerVisible(false);
                }
                self.toggleExtraPalette = function() {
                    if (self.isExtraPaletteVisible()) {
                        self.colors(self.colorPalette)
                    } else {
                        self.colors(self.extraColorPalette)
                    }
                    self.isExtraPaletteVisible(!self.isExtraPaletteVisible());
                }
            }
        });
    }
