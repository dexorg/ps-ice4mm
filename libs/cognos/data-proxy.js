/**
 * adapted from https://github.com/IBM/cognos-dashboard-demo/blob/master/lib/daasProxy.js
 *
 */

const logFormatter = require('dex-logger').logFormatter;

const logger = logFormatter('cognos-proxy','req');
    ...
    if (!req.secure && headers && headers['set-cookie']) {
        ...
        for (let i=0; i < setCookies.length; i++) {
            ...
            if (cookie && cookie.indexOf('Secure') > 0) {
                ...
                setCookies[i] = cookie;
                logger.info('Stripped Secure from a cookie');
            }
        }
...
function resetRequestStream(req) {
    ...
    if (isJson && req.body && req.method !== 'GET' && req.method !== 'DELETE' ) {
        ...
        logger.info('forwarding json body. len=' + bodyData.length);
        return s;
    } else {
        return req;
    }
...
module.exports=function(proxyTarget) {
    return function(req, res/*, next*/) {
        ...
        logger.info('Proxy request: ' + req.method + ' ' + routingURL);
        ...
        resMsg.on('response', function(response) {
            logger.info('Proxy response: ' + response.statusCode);
